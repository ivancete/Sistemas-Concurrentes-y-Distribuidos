...
void Camarero(int id)
{
	...
	while(1)
	{
		if(num_filosofos == 4)
			rama = 1;
		else if(num_filosofos == 0)
			rama = 0;
		else
		{
			MPI_Probe( MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &estado );

			if(estado.MPI_TAG == 50 && num_filosofos < 4)
			{	//Con el tag 50 reconozco a los que quieren sentarse
				rama = 0;
			}
			else{	//Con el tag 51 reconozco a los que quieren levantarse
				rama = 1;
			}
		}
		...
		{
			case 0:
				MPI_Recv( &basura, 1, MPI_INT, MPI_ANY_SOURCE,50, MPI_COMM_WORLD, &estado );
	            cout << "Camarero deja sentarse al filosofo " << estado.MPI_SOURCE << endl << flush;
	            num_filosofos++;  
	            cout << "++ " << num_filosofos << endl << flush; 
	            break;

	        case 1:
	        	MPI_Recv( &basura, 1, MPI_INT, MPI_ANY_SOURCE,51, MPI_COMM_WORLD, &estado );
	            cout << "Camarero deja levantarse al filosofo " << estado.MPI_SOURCE << endl << flush;  
	            num_filosofos--;
	            cout << "-- " << num_filosofos << endl << flush;
	            break;
		}
