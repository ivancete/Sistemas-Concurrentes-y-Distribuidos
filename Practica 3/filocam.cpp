...
void Filosofo( int id, int nprocesos )
{
   ...
   while(1)
   {
        MPI_Ssend(&basura,1,MPI_INT,10,0,MPI_COMM_WORLD);
        MPI_Recv(&basura,1,MPI_INT,10,0,MPI_COMM_WORLD,&status);
	    ...
	    else
	    {
	    	// Solicita tenedor derecho
	        cout <<"Filosofo "<<id<< " coge tenedor der ..." << der << endl << flush;
	        MPI_Ssend(&basura, 1, MPI_INT, (nprocesos + id - 2)%nprocesos, (nprocesos + id - 2)%nprocesos, MPI_COMM_WORLD);
	        MPI_Recv(&basura,1,MPI_INT, (nprocesos + id - 2)%nprocesos,(nprocesos + id - 2)%nprocesos, MPI_COMM_WORLD, &status);
	    ...
	    if(id==0){
	        // Suelta el tenedor derecho(especial para el 0)
	        cout <<"Filosofo "<<id<< " suelta tenedor der ..." << der << endl << flush;
	        MPI_Ssend(&basura, 1, MPI_INT, (nprocesos + id - 2)%nprocesos, 2*((nprocesos + id - 2)%nprocesos), MPI_COMM_WORLD);
	    }else{
	        // Suelta el tenedor derecho (en general)
	       	cout <<"Filosofo "<<id<< " suelta tenedor der ..." << der << endl << flush;
	        MPI_Ssend(&basura, 1, MPI_INT, (nprocesos + id - 1)%nprocesos, 2*((nprocesos + id - 1)%nprocesos), MPI_COMM_WORLD);
        ...
        MPI_Ssend(&basura,1,MPI_INT,10,1,MPI_COMM_WORLD);
        MPI_Recv(&basura,1,MPI_INT,10,1,MPI_COMM_WORLD,&status);
...
void Tenedor(int id, int nprocesos)
{
  ...
  while( 1 )
  {
    ...
    MPI_Ssend(&buf,1, MPI_INT, Filo, id, MPI_COMM_WORLD);
    
    cout << "Ten. " << id << " recibe petic. de " << Filo << endl << flush;
    
    // Espera a que el filosofo suelte el tenedor...
    MPI_Recv(&buf,1,MPI_INT, MPI_ANY_SOURCE,2*id, MPI_COMM_WORLD, &status);
...
void Camarero (int id, int nprocesos)
{
  //tag 0 para sentarse, tag 1 para levantarse
   ...
   while(1)
   {
      ...
      case 1:
         ...
         MPI_Ssend(&buf,1,MPI_INT,filo,1,MPI_COMM_WORLD);
